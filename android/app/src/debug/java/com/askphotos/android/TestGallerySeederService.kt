...
class TestGallerySeederService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val invalidOperation = operationId != null && !TestGallerySeederReceiver.CLEANUP_OPERATION_ID.matches(operationId)
        if (action !in setOf(ACTION_SEED, ACTION_CLEANUP) || runId == null ||
        ...
        ServiceCompat.startForeground(
            this,
            NOTIFICATION_ID,
            notification(if (action == ACTION_SEED) "Preparing $runId" else "Cleaning $runId", 0, 0),
        ...
        if (seedJob?.isActive == true) {
            if (activeRunId != runId || activeAction != action) {
                TestGallerySeederReceiver().writeStatus(
                    this,
                    runId,
                    if (action == ACTION_CLEANUP) "cleanup-status.json" else "status.json",
        ...
        seedJob = scope.launch {
            try {
                if (action == ACTION_SEED) {
                    ...
                    TestGallerySeederReceiver().cleanup(this@TestGallerySeederService, runId, operationId)
                }
                getSystemService(NotificationManager::class.java).notify(
                    NOTIFICATION_ID,
                    notification(if (action == ACTION_SEED) "Test gallery ready" else "Test gallery removed", 1, 1),
                )
    ...
    companion object {
        ...
        fun start(context: Context, runId: String, action: String = ACTION_SEED) {
            require(TestGallerySeederReceiver.RUN_ID.matches(runId))
            require(action == ACTION_SEED || action == ACTION_CLEANUP)
            ContextCompat.startForegroundService(
                context,
                Intent(context, TestGallerySeederService::class.java).setAction(action).putExtra(EXTRA_RUN_ID, runId),
            )
        }
    }
}
