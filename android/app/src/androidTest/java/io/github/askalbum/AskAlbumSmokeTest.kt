...
class AskAlbumSmokeTest {
    ...
    fun privacyCanOpenOnboardingAndReturnToAsk() {
        ...
        rule.onNodeWithText("Review onboarding").performScrollTo().performClick()
        rule.waitUntil(timeoutMillis = 10_000) {
            runCatching { rule.onNodeWithContentDescription("Onboarding screen").fetchSemanticsNode() }.isSuccess
        }
