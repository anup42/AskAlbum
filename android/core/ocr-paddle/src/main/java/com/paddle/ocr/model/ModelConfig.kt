...
data class ModelConfig(
    ...
    companion object {
        ...
        fun parse(file: File): ModelConfig {
            val content = try {
                require(file.isFile) { "Recognition config does not exist" }
                file.readText()
            } catch (t: Throwable) {
                throw OCRError.ConfigParseFailed(file.name, t)
            }
            return parseContent(content, file.name)
        }

        private fun parseContent(content: String, sourceName: String): ModelConfig {
            val characterDict = try {
                extractCharacterDict(content, sourceName)
            } catch (e: OCRError.ConfigParseFailed) {
                throw e
            } catch (t: Throwable) {
                throw OCRError.ConfigParseFailed(sourceName, t)
            }
