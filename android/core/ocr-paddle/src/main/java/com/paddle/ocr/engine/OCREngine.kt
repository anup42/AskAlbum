...
class OCREngine(
    ...
    init {
        val configured = try {
            val fileMode = listOf(detModelFile, recModelFile, recConfigFile)
            require(fileMode.all { it == null } || fileMode.all { it != null }) { "OCR model files must be supplied together" }
            val recModelConfig = if (detModelFile != null) {
                ortManager.loadModelFiles(detModelFile, requireNotNull(recModelFile))
                ModelConfig.parse(requireNotNull(recConfigFile))
            } else {
