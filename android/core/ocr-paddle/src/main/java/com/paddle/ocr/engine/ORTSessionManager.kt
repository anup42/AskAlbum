...
class ORTSessionManager(
    ...
    fun loadModels(detAssetPath: String, recAssetPath: String) {
        loadModels(readModelAsset(detAssetPath), readModelAsset(recAssetPath))
    }

    fun loadModelFiles(detModelFile: File, recModelFile: File) {
        loadModels(readModelFile(detModelFile), readModelFile(recModelFile))
    }

    private fun loadModels(detBytes: ByteArray, recBytes: ByteArray) {
    ...
    private fun readModelFile(file: File): ByteArray = try {
        require(file.isFile) { "Model file does not exist" }
        file.readBytes()
    } catch (t: Throwable) {
        throw OCRError.ModelNotFound(file.name, t)
    }
