...
| SigLIP2 vector retrieval | **Installed; tokenizer parity and semantic smoke passed; core Recall@K pending** | LiteRT 2.1.0 and ONNX Runtime Mobile 1.23.2 dual-encoder paths, pinned pack tooling, exported tokenizer contract, APK-key signature verification, per-file hashes, exact vector scan, and RRF are present. The signed q8 base/224 pack now matches official SentencePiece BPE token IDs and passed red/blue plus natural dog/football ranking on-device. A calibrated core threshold pack was installed and rechecked, but the Q01-Q13 run was interrupted by reference-device disconnection. |
...
The application is now a functional Android gallery indexer and evidence-backed search product, not merely a UI mock. It is not yet the complete production system described in the supplied document. The consumer app fetched and verified the official Gemma 4 E2B LiteRT-LM artifact; real multilingual planning, targeted visual verification, grounded composition, and no-answer gates passed on-device. E4B remains optional and untested. The pinned q8 SigLIP2 pack now has accepted tokenizer parity and bounded semantic smoke results; full core Recall@K/threshold evaluation and 5k/20k measurements remain pending. Other blockers include a licensed face-embedding model and Q07/Q08 identity evaluation under explicit consent.
